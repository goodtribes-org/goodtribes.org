...
export default function HeroCarouselDialog({
  slides,
  onSlidesChange,
  onClose,
}: {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      ...
      onSlidesChange(editingId === "new" ? [...slides, saved] : slides.map((s) => (s.id === saved.id ? saved : s)));
      setEditingId(null);
    });
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <h2 className="text-lg font-bold text-dark-slate mb-1">Hero-karusell</h2>
        <p className="text-sm text-dark-slate/50 mb-4">
          Bilderna och texterna som visas i karusellen överst på startsidan. Bara site-admins kan ändra dem.
        </p>

        <ul className="space-y-2 mb-4">
          {slides.map((s, i) => (
            <li key={s.id} className="flex items-center gap-2 text-sm border border-dark-slate/10 rounded-lg px-2 py-2">
              <img src={toProxyUrl(s.imageUrl)} alt="" className="w-12 h-8 object-cover rounded-md flex-shrink-0" />
