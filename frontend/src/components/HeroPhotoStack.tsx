    ...
    <div className="relative">
      ...
      <div className="relative z-10 flex justify-center px-4 pt-0 pb-6">
        <div className="flex w-full max-w-6xl flex-col items-center gap-3">
          ...
          <div className="grid w-full gap-8 items-center md:grid-cols-2 py-6">
            {/* Bild — till vänster */}
            <div
            ...
            >
              <div className={`relative h-full bg-white p-3 ${CARD_SHADOW}`}>
                ...
                <div className={`h-full md:overflow-y-auto border border-muted-teal/20 px-6 pt-3 pb-5 flex flex-col justify-start ${heroTintClass(first.tintColor, first.tintOpacity)}`}>
                  <div className="flex flex-col items-start text-left">
                    <h2 className={`${handwritingFont.className} text-dark-slate pr-16`} style={{ textWrap: "balance", fontSize: 26 }}>
                      {first.heading}
                    </h2>
                    <RichText html={first.body} className="mt-1 text-dark-slate" />
                    {first.bodyLine2 && (
                      <p className="mt-1 font-semibold text-dark-slate" style={{ fontSize: 14, lineHeight: 1.5 }}>
                        {first.bodyLine2}
                      </p>
                    )}
                    {first.obstacles.length > 0 && (
                      <>
                        <ul className="mt-3 flex flex-col gap-2.5">
                          {first.obstacles.map((o) => (
                            <li key={o.lead} className="text-sm text-dark-slate">
                              <span className="font-bold text-seagrass">{o.lead}</span> {o.text}
                            </li>
                          ))}
                        </ul>
                        {first.outro && <RichText html={first.outro} className="mt-3 text-dark-slate" />}
                      </>
                    )}
