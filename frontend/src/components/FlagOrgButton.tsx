...
export default function FlagOrgButton({ organisationId }: { organisationId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/org-flags", {
        ...
        body: JSON.stringify({ organisationId, reason: `${reason}: ${motivering.trim()}` }),
      });
      if (!res.ok) {
  ...
  if (!open) {
    return (
      <button
        ...
        title="Flagga denna organisation för granskning"
      >
        <span aria-hidden>⚑</span> Flagga organisation
      </button>
    );
  }
  ...
  return (
    <div className="mt-2 border border-muted-teal/40 rounded-lg p-4 bg-white max-w-sm">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-dark-slate">Flagga organisation</h3>
        <button
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Beskriv varför du flaggar denna organisation..."
