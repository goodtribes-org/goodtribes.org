...
export default function ResourceLibrary({ organisationId, projectId, files, canUpload }: Props) {
  ...
  async function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error((data as { error?: string }).error ?? t("uploadFailed"));
      }
      router.refresh();
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {files.length === 0 ? (
        <p className="text-dark-slate/40 italic text-sm">{t("noResourcesYet")}</p>
      ) : (
        <div className="flex flex-col divide-y divide-muted-teal/20 border border-muted-teal/30 rounded-lg overflow-hidden">
          {files.map((f) => (
            ...
            >
              ...
              <span className="text-xs text-coral flex-shrink-0">{t("download")}</span>
            </a>
          ))}
        </div>
      )}
