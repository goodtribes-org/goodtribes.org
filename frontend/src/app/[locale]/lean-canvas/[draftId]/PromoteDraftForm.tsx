...
export default function PromoteDraftForm({ draftId, suggestedTitle }: { draftId: string; suggestedTitle: string }) {
  ...
  function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      const result = await promoteLeanCanvasDraftToProject(draftId, formData);
      if ("slug" in result) {
        router.push(`/projects/${result.slug}/lean-canvas`);
      } else {
