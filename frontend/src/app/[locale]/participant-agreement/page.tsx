...
export async function generateMetadata({ params }: { params: Promise<{ locale: Locale }> }): Promise<Metadata> {
  ...
  const page = (await getSitePage("participant-agreement", locale)) ?? DEFAULT_SITE_PAGES["participant-agreement"][locale];
...
export default async function ParticipantAgreementPage({ params }: { params: Promise<{ locale: Locale }> }) {
  const { locale } = await params;
