...
import { castCouncilVote } from "./actions";
...
export default function CouncilVoteForm({ pollId, candidates, gtBalance, existingVotes }: Props) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (allocations.length === 0) {
      setError("Fördela minst ett GT på en kandidat.");
      return;
    }
    ...
    startTransition(async () => {
      const result = await castCouncilVote(pollId, allocations);
  ...
  if (candidates.length === 0) {
    return <p className="text-sm text-dark-slate/40">Inga kandidater har nominerat sig ännu.</p>;
  }

  return (
    <div className="border border-muted-teal rounded-lg p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold text-dark-slate">Rösta</h2>
