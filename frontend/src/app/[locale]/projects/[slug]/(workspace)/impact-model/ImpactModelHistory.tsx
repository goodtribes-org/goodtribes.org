...
import { getImpactModelHistory } from "./actions";
...
} & Record<ImpactModelField, string | null>;

function formatDate(d: Date) {
  return new Date(d).toLocaleString("sv-SE", { dateStyle: "medium", timeStyle: "short" });
}

// Same modal as LeanCanvasHistory (and its UI strings), for the impact model.
export default function ImpactModelHistory({ projectSlug }: { projectSlug: string }) {
  const t = useTranslations("LeanCanvasHistory");
  const tField = useTranslations("ImpactModelPage");
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [versions, setVersions] = useState<Version[] | null>(null);
  const [selected, setSelected] = useState<Version | null>(null);

  const label = (key: string) => tField(`field${key}` as Parameters<typeof tField>[0]);
  ...
  async function handleOpen() {
    ...
    if (versions) return;
    setLoading(true);
    const data = await getImpactModelHistory(projectSlug);
            ...
            {!selected ? (
              ...
              <>
                ...
                <div className="space-y-3">
                  {IMPACT_MODEL_BLOCKS.map(({ field, translationKey }) => (
                    <div key={field}>
