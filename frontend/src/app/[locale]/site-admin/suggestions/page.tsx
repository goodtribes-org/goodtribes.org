...
export default async function SuggestionsAdminPage({ params }: { params: Promise<{ locale: Locale }> }) {
  ...
  const [suggestions, t] = await Promise.all([
    ...
    getTranslations({ locale, namespace: "SuggestionsAdminPage" }),
  ]);
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-4">
      <div className="mb-8">
        ...
        <p className="text-sm text-dark-slate/60 mt-1">{t("intro", { count: suggestions.length })}</p>
      </div>
          ...
          {suggestions.map((s) => (
            <div key={s.id} className="border border-muted-teal/40 rounded-lg p-5 bg-white">
              ...
              <p className="text-xs text-dark-slate/50 mb-4">
                {t("fromLabel")} <span className="font-medium text-dark-slate/70">{s.author.name ?? s.author.email}</span>
              </p>
