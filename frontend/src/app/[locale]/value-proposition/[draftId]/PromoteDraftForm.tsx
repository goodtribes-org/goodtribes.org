...
import { promoteValuePropositionDraftToProject } from "../actions";

export default function PromoteDraftForm({ draftId, suggestedTitle }: { draftId: string; suggestedTitle: string }) {
  const t = useTranslations("ValuePropositionDraftPage");
  ...
  function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      const result = await promoteValuePropositionDraftToProject(draftId, formData);
      if ("slug" in result) {
        router.push(`/projects/${result.slug}/value-proposition`);
      } else {
