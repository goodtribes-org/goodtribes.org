...
export default async function ValuePropositionListPage({
  params,
}: {
  ...
  // viewer's own, same as /lean-canvas and /whiteboard.
  const drafts = await prisma.valuePropositionDraft.findMany({
          ...
          {drafts.map((d) => (
            ...
            >
              <div className="min-w-0">
                <p className="text-sm font-medium text-dark-slate truncate">{d.name || d.vpJobs?.slice(0, 80) || t("listStartedBy", { name: d.owner.name ?? t("unknownAuthor") })}</p>
