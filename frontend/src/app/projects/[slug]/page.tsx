        ...
        <div className="col-span-3">
          <div className="relative w-full aspect-video bg-dark-slate rounded overflow-hidden">
            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-dark-slate/80 to-dark-slate text-white text-center px-8">
              <p className="text-2xl font-bold leading-snug">{project.title}</p>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-2 flex flex-col gap-4">
          ...
          {project.sdgGoals.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {project.sdgGoals.map((n) => (
                <span
                  key={n}
                  className="text-xs bg-dry-sage text-dark-slate px-2 py-0.5 rounded"
                  title={SDG_LABELS[n]}
                >
                  SDG {n}
                </span>
          ...
          {totalTasks > 0 && (
            <div>
              ...
              <ProgressBar value={totalTasks} max={totalTasks} color="#43aa8b" />
            </div>
          )}
        ...
        <div className="col-span-3">
          ...
          <div className="border-b border-muted-teal/40 mb-6">
            <div className="flex gap-6">
              <button className="pb-3 text-sm font-medium border-b-2 border-coral text-coral whitespace-nowrap">
                Story
              </button>
              <Link
          ...
          <div className="border border-muted-teal/30 rounded p-4 mb-6">
                ...
                <div key={stage} className="flex-1 flex flex-col items-center z-10">
                  <div
                    className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                      i <= stageIndex ? "bg-seagrass border-seagrass" : "bg-white border-gray-300"
                    }`}
                  >
                    {i <= stageIndex && (
            ...
            <div className="flex">
              {STAGES.map((stage, i) => (
                <div key={stage} className="flex-1 text-center">
                  <span
                    className={`text-xs ${
                      i <= stageIndex ? "text-dark-slate font-medium" : "text-dark-slate/40"
                    }`}
                  >
                    {stage}
                  </span>
            ...
            <div className="mt-4 pt-3 border-t border-muted-teal/20">
              <p className="text-xs font-semibold text-dark-slate/50 uppercase tracking-wider mb-1">
                Highlights
              </p>
          ...
          <div className="prose prose-sm max-w-none text-dark-slate/80 leading-relaxed">
            {project.description ? (
              ...
              <p className="text-dark-slate/40 italic">No description yet.</p>
            )}
          </div>
        </div>
            ...
            {project.members.length > 0 ? (
              <div className="grid grid-cols-4 gap-3">
                {project.members.map((m) => (
                  <MemberAvatar key={m.id} name={m.user.name ?? "?"} />
                ))}
              </div>
            ) : (
