                      ...
                      {sortedMembers.map((m, i) => {
                        ...
                        return (
                          <Tooltip key={i} lines={[m.user.name ?? "?", ...(isProjectOwner ? ["Founder"] : [])]}>
                            <div
                              className={`w-10 h-10 rounded-full overflow-hidden bg-dry-sage relative flex items-center justify-center text-sm font-semibold text-dark-slate shrink-0 ring-2 transition-transform duration-200 ease-in-out hover:scale-[1.6] cursor-pointer ${isProjectOwner ? "ring-seagrass" : "ring-white"}`}
                            >
                              {m.user.image ? (
                                <Image
                                  src={m.user.image}
                    ...
                    <div className="grid grid-cols-7 gap-0.5">
                      {[...project.sdgGoals, 18].map((n) => (
                        <Tooltip key={n} lines={[`SDG ${n}`, SDG_LABELS_SV[n] ?? ""]}>
                          <div className="transition-transform duration-200 ease-in-out hover:scale-[1.6] cursor-pointer">
                            <SdgIcon n={n} size={38} />
                          </div>
                        </Tooltip>
                      ))}
