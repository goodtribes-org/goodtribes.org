  ...
  it("keeps at most 3 points, drops unknown field keys but keeps the point", () => {
    const c = coerceCritique({
      points: [
        ...
        { text: "", field: "leanCanvas.jobsToBeDone", severity: "high" },
...
describe("coerceSynthesis — grounded in the interviews", () => {
  const assumptions = new Set(["leanCanvas.jobsToBeDone", "valueProposition.vpPains"]);
  ...
  it("only keeps verdicts on assumptions we asked about, citing real interviews", () => {
    const s = coerceSynthesis(
      {
        ...
        verdicts: [
          { field: "leanCanvas.jobsToBeDone", verdict: "confirmed", reason: "Alla tre", interview_ids: ["i1", "i2", "i3", "ghost"] },
          ...
          { field: "leanCanvas.jobsToBeDone", verdict: "refuted", reason: "Dubblett", interview_ids: ["i1"] },
        ],
      },
      assumptions,
      interviews,
    ...
    expect(s.verdicts).toEqual([
      { field: "leanCanvas.jobsToBeDone", verdict: "confirmed", reason: "Alla tre", interviewIds: ["i1", "i2", "i3"] },
