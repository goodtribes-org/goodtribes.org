...
await pageA.goto(`${BASE}/members/${B.id}`, { waitUntil: "networkidle" });
...
await pageB.goto(`${BASE}/messages`, { waitUntil: "networkidle" });
...
const replyTextarea = pageB.locator("textarea").first();
...
const kanbanResp = await pageA.goto(`${BASE}/projects/goodtribes/kanban`, { waitUntil: "networkidle" });
results.kanbanFinalUrl = pageA.url();
results.kanbanStatus = kanbanResp?.status();

const milestonesResp = await pageA.goto(`${BASE}/projects/goodtribes/milestones`, { waitUntil: "networkidle" });
