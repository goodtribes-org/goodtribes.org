...
- **Kritisk sidoupptäckt och fix:** att lägga till `guardSocialAction`-anrop på dessa fyra ställen exponerade att `checkRateLimit` (`frontend/src/lib/rateLimit.ts`) saknade en timeout — när Redis inte är nåbart (vilket är fallet både i lokal dev utan Redis och i produktion, se `REDIS_URL`-gapet i Known Issues) tar ioredis default-retries **15–30 sekunder per anrop** innan det ger upp, vilket i praktiken gjorde idé-skapande, idékommentarer och chattmeddelanden orimligt långsamma (uppmätt: 20s per requests i denna sessions manuella test) istället för att "fail open" som avsett. Fixat med en 250ms timeout-race runt Redis-anropet i `checkRateLimit` — påverkar alla befintliga användare av `guardSocialAction` (like/comment/flag), inte bara den nya koden.
...
| `sprint` | **(v4.20, korrigerar v4.19: behåller v4.18:s innehåll, lägger till nytt)** `map_understand` ("Kartlägga & förstå"), `sketch_solutions` ("Skissa lösningar"), `decide_plan` ("Beslut & planera"), `build_prototype` ("Bygga prototyp"), `test_with_users` ("Testa med användare"), `sprint_lean_canvas` ("Skriva ett enkelt lean canvas / projektbeskrivning", länkar till Lean Canvas), `core_team_formed` ("Definiera roller och bilda kärnteam"), `kanban_seeded` ("Sätta upp Kanban-board med första uppgifterna", länkar till Kanban), `pilot_success_criteria` ("Definiera framgångskriterier för pilotfasen"), `rough_budget_estimated` ("Ta fram grov budget/resursbehov") |
| `pilot` | **(v4.20, korrigerar v4.19: behåller v4.18:s innehåll, lägger till nytt)** `todo_created` ("Fyll på med arbetsuppgifter"), `collaborators_invited` ("Bjud in medskapare"), `team_formed` ("Formera team"), `resources_secured` ("Säkra resurser"), `pilot_scope_defined` ("Avgränsa ett litet, konkret test (tid, plats, målgrupp)"), `pilot_executed_documented` ("Genomföra piloten och dokumentera lärdomar löpande"), `pilot_results_collected` ("Samla in kvantitativa/kvalitativa resultat"), `pilot_model_adjusted` ("Justera modellen baserat på feedback"), `pilot_go_no_go` ("Utvärdera mot framgångskriterierna → go/no-go beslut") |
...
impact_reports
  ...
  kind (delivered | support_received), value_qualifier (exact | at_least | approximate)
  is_cumulative, source_name
...
2. ~~Bygg `impact_reports`-tabellen~~ **Klart (v4.12)** — `ImpactReport`-modellen finns nu i schemat. ~~Kvarstår: UI för att skapa och verifiera rapporter~~ **Klart (2026-09-01)** — hela kedjan är byggd: projektledare skickar in ett resultat med SDG-koppling, period och länk till underlag från projektets impact-sida, site-admin verifierar eller avslår (med obligatorisk motivering) i en granskningskö på `/site-admin/impact-reports`, och verifierade resultat visas publikt på projektsidan. Modellen utökades additivt med `metric_unit`, `period_start`/`period_end`, `evidence_url`, `created_by`, `rejected_at` och `review_note` — utan underlag och motivering vore "verifierad" bara samma självrapporterade siffra en gång till. Verifieringsrätten lades på site-admin (Stiftelsen), inte Granskningsrådet, eftersom rådets mandat enligt 5.54 är reaktivt (anmälningar om regelbrott) medan detta är proaktiv kvalitetsgranskning — flytten dit är dock ett enradsbyte i `site-admin/impact-reports/actions.ts` om styrelsen beslutar annat. **Utökad samma dag** efter att Föreningen INFOS verkliga historik (25 000+ donerade datorenheter, 50+ MSEK i utrustning, 1 658 000 kr från Stockholms stad 2005–2008, 2 MSEK i sponsring) modellerats mot fältuppsättningen: fyra av sex uppgifter gick inte att uttrycka sanningsenligt. Tillagt `kind` (levererat resultat vs mottaget stöd — bidrag och sponsring är resurser *in*, aldrig effekt *ut*, och visas i egen underordnad sektion), `is_cumulative` (en "totalt sedan start"-siffra *överlappar* periodrapporterna i stället för att adderas till dem), `value_qualifier` (så att "över 25 000" inte lagras och visas som exakt 25 000) och `source_name` (Stockholms stad, Coop/OK/Q8, "näringsliv och offentlig sektor"). Konsekvens att känna till: plattformen summerar medvetet aldrig impact-rapporter — varje totalsumma den räknade fram själv skulle dubbelräkna en kumulativ rapport mot dess egna periodrapporter. Kvarstår fortfarande: att koppla rapporterna till den faktiska `scale`→`impact`-gatinglogiken när den beslutas (se punkt 1).
